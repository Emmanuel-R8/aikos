...
(defun test-init-destroy-display ()
  ...
  (let ((display (laiko.display:init-display 640 480)))
    (assert display nil "Display should be created")
    (assert (= (laiko.display:display-width display) 640) nil "Width should be 640")
    (assert (= (laiko.display:display-height display) 480) nil "Height should be 480")
    (laiko.display:destroy-display display)
    (format t "  Display destroyed successfully~%")))

(defun test-render-region ()
  "Test rendering a region to display"
  (let ((display (laiko.display:init-display 100 100))
        (buffer (make-array (* 10 10 4) ; 10x10 RGBA
                            :element-type '(unsigned-byte 8)
                            :initial-element #xFF))) ; White
    ;; Render region at (10, 10) with size 10x10
    (laiko.display:render-region display 10 10 10 10 buffer)
    ;; Verify buffer was copied (check a pixel)
    (let ((display-buffer (laiko.display:display-buffer display))
          (offset (+ (* 10 100 4) (* 10 4)))) ; (10, 10) in 100x100 display
      (assert (= (aref display-buffer offset) #xFF) nil "Pixel should be white"))
    (laiko.display:destroy-display display)))
...
(defun test-bitblt-copy ()
  ...
  (let ((display (laiko.display:init-display 100 100)))
    ...
    (let ((buffer (laiko.display:display-buffer display)))
      ...
      (laiko.display:bitblt display 10 10 20 20 50 50 0) ; 0 = COPY
