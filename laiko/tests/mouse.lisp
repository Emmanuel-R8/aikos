...
(defun test-update-get-mouse-position ()
  ...
  (maiko-lisp.io:update-mouse-position 100 200)
  (multiple-value-bind (x y)
      (maiko-lisp.io:get-mouse-position)
...
(defun test-translate-mouse-event ()
  ...
  (maiko-lisp.io:update-mouse-position 50 75)
  (let ((os-event nil)) ; Stub OS event
    (let ((mouse-event (maiko-lisp.io:translate-mouse-event os-event)))
      (assert mouse-event nil "Mouse event should be created")
      (assert (= (maiko-lisp.io:mouse-x mouse-event) 50) nil "X should match current position")
      (assert (= (maiko-lisp.io:mouse-y mouse-event) 75) nil "Y should match current position"))))

(defun test-mouse-event-structure ()
  "Test mouse event structure creation"
  (let ((event (maiko-lisp.io:make-mouse-event
                :x 10
                :y 20
                :buttons 1
                :pressed-p t)))
    (assert (= (maiko-lisp.io:mouse-x event) 10) nil "X should be 10")
    (assert (= (maiko-lisp.io:mouse-y event) 20) nil "Y should be 20")
    (assert (= (maiko-lisp.io:mouse-buttons event) 1) nil "Buttons should be 1")
    (assert (maiko-lisp.io:mouse-pressed-p event) nil "Pressed flag should be true")))
