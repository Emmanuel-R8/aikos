...
(defun test-translate-keycode-ascii ()
  ...
  (let ((ascii-key (char-code #\A))) ; 'A' = 65
    (let ((lisp-keycode (laiko.io:translate-keycode ascii-key)))
...
(defun test-translate-keycode-extended ()
  ...
  (let ((extended-key 200)) ; Extended key
    (let ((lisp-keycode (laiko.io:translate-keycode extended-key)))
      (assert (>= lisp-keycode #x80) nil "Extended keycode should be >= 0x80"))))

(defun test-enqueue-dequeue-key-event ()
  "Test keyboard event queue"
  (let ((event (laiko.io:make-keyboard-event
                :keycode 65
                :modifiers 0
                :pressed-p t)))
    ;; Enqueue event
    (laiko.io:enqueue-key-event event)
    ;; Dequeue event
    (let ((dequeued (laiko.io:dequeue-key-event)))
      (assert dequeued nil "Event should be dequeued")
      (assert (= (laiko.io:kbd-keycode dequeued) 65) nil "Keycode should match")
      (assert (laiko.io:kbd-pressed-p dequeued) nil "Pressed flag should match"))))
