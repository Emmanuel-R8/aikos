...
(defun trace-log (vm pc opcode operands &key instruction-name)
  ...
  (let* ((sp-bytes (vm-stack-ptr-offset vm))
         ;; Use DLword unit for SP/FP display to match C emulator
         (sp-dlword (ash sp-bytes -1))
         (fp-dlword (ash (laiko.vm:vm-frame-pointer-offset vm) -1))
         (tos (laiko.vm:vm-tos vm))
         ;; Stack peek needs to be robust (TODO: Implement proper stack peek)
         (n1 0) 
         (n2 0) 
         (page-num (ash pc -9))
    ...
    ;; PC (0x%06x format - 6 hex digits with leading zeros, lowercase)
    ...
    (format *vm-trace-output* "SP:0x~(~6,'0x~),FP:0x~(~6,'0x~)|" sp-dlword fp-dlword)
    ...
    (let ((vp page-num)
          (va (ash page-num 9)))
       (format *vm-trace-output* "FP:0,VP:~D,FO:0x0,VA:0x~(~x~)|" vp va))
    ;; BS_MEM
    (format *vm-trace-output* "BS:RAW,MEM:????????|")
    ;; NOTES (empty for now)
