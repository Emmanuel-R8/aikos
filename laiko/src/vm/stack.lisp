...
(defstruct (vm (:conc-name vm-))
  ...
  (ifpage nil :type (or null laiko.data:ifpage))
...
(defun vm-read-word (vm byte-offset)
  ...
  (declare (type vm vm)
            (type (unsigned-byte 32) byte-offset))
  (let* ((vmem (vm-virtual-memory vm))
         ;; Maiko GETWORD on BYTESWAP builds uses address XOR 2.
         (physical-byte-offset (if (laiko.utils:little-endian-p)
                                   (logxor byte-offset 2)
                                   byte-offset)))
    (unless vmem
      (return-from vm-read-word 0))
    (let* ((page-num (ash physical-byte-offset -9))
           (page-offset (logand physical-byte-offset #x1FF)))
      (when (>= page-num (length vmem))
        (return-from vm-read-word 0))
      (let ((page (aref vmem page-num)))
        (if (null page)
...
(defun vm-write-word (vm byte-offset value)
  ...
  (let* ((vmem (vm-virtual-memory vm))
         ;; Maiko GETWORD/PUTWORD on BYTESWAP builds use address XOR 2.
         (physical-byte-offset (if (laiko.utils:little-endian-p)
                                   (logxor byte-offset 2)
                                   byte-offset)))
    (unless vmem
