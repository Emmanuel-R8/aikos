...
pub fn executeInstructionInLoop(
    ...
    // CRITICAL: Sync CSTKPTRL from CurrentStackPTR before each instruction (matching C StackPtrRestore)
    // C: StackPtrRestore = CSTKPTRL = (void *)(CurrentStackPTR + 2)
    // This ensures TOPOFSTACK is read from correct memory location after operations like UNBIND
    stack.initCSTKPTRLFromCurrentStackPTR(vm);

    // CRITICAL: Also read TOPOFSTACK from memory (matching C behavior)
    // C: TOPOFSTACK = *(CSTKPTRL - 1) - reads actual value from memory
    ...
    if (jump_offset) |offset| {
        ...
        // TODO: Re-enable debug printing when debug functions are stable
        // For now, skip debug printing to avoid compilation issues
    } else {
        vm.pc += inst.length;
...
pub fn decodeInstruction(vm: *VM, instruction_count: u64) errors.VMError!?instruction.Instruction {
    ...
    if (instruction_count <= 3) {
        const opcode_val: u32 = @intFromEnum(inst.opcode);
        const opcode_u8: u8 = @intCast(opcode_val & 0xFF);
        dispatch_debug.printDecodedInstruction(@as(usize, @intCast(vm.pc)), opcode_u8, @tagName(inst.opcode), @as(u8, @intCast(inst.length)), instruction_count);
    }
