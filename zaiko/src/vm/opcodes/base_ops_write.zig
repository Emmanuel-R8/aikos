    ...
    if (segment == types.S_POSITIVE) {
        ...
        if (type_num) |tn| {
            if (tn == type_check_module.TYPE_FIXP) {
                ...
                const virtual_memory_fixp = vm.virtual_memory orelse return errors_module.VMError.MemoryAccessFailed;
                const fixp_ptr = virtual_memory_module.translateAddress(virtual_memory_fixp, byteoffset_value, fptovp_table_fixp, 4) catch {
...
pub fn handlePUTBASE_N(vm: *VM, index: u8) errors.VMError!void {
    ...
    const native_ptr = virtual_memory_module.translateAddress(virtual_memory_mut, base_ptr + @as(LispPTR, index), fptovp_table, 2) catch {
        return errors_module.VMError.InvalidAddress;
    };
...
pub fn handlePUTBASEPTR_N(vm: *VM, index: u8) errors.VMError!void {
    ...
    const native_ptr = virtual_memory_module.translateAddress(virtual_memory_mut, base_ptr + @as(LispPTR, index), fptovp_table, 4) catch {
        return errors_module.VMError.InvalidAddress;
    };
...
pub fn handlePUTBITS_N_FD(vm: *VM, arg1: u8, arg2: u8) errors.VMError!void {
    ...
    const native_ptr = virtual_memory_module.translateAddress(virtual_memory_mut, base_ptr + @as(LispPTR, arg1), fptovp_table, 2) catch {
        return errors_module.VMError.InvalidAddress;
    };
