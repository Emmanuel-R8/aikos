...
/// Per rewrite documentation instruction-set/opcodes-data.md
/// C: TYPEP(n) macro in maiko/inc/inlineC.h
/// Checks if TOS value's type number matches the given type_code operand
/// 
/// **C Implementation**:
/// ```c
/// #define TYPEP(n) \
...
/// 
/// **Behavior**:
/// - Gets type number of TOS using GetTypeNumber()
/// - Compares with operand type_code (1 byte)
/// - If types match: TOS remains unchanged (value stays on stack)
/// - If types don't match: TOS is set to NIL_PTR
/// 
/// **Type Number Lookup**:
/// - GetTypeNumber(address) = GetTypeEntry(address) & 0x7ff
/// - GetTypeEntry(address) = GETWORD(MDStypetbl + ((address) >> 9))
/// - Type number is low 11 bits of type entry from type table
/// 
/// **Stack Effect**: [value] -> [value or NIL]
/// **Instruction Format**: [opcode:0x05][type_code:1B] (length: 2 bytes)
pub fn handleTYPEP(vm: *VM, type_code: u8) errors.VMError!void {
    const stack_module = @import("../stack.zig");
    const type_check_module = @import("../../utils/type_check.zig");
    const value = stack_module.getTopOfStack(vm);

    // C: GetTypeNumber(TOPOFSTACK)
    const type_num = type_check_module.getTypeNumber(vm, value);

    // C: if ((DLword)GetTypeNumber(TOPOFSTACK) != (n)) TOPOFSTACK = NIL_PTR;
    // If type number matches type_code, leave TOS unchanged
    // If type number doesn't match, set TOS to NIL_PTR
    if (type_num) |tn| {
        if (@as(u16, tn) != type_code) {
            // Type doesn't match - set to NIL
            stack_module.setTopOfStack(vm, type_check_module.NIL_PTR);
        }
        // Type matches - leave TOS unchanged (value stays on stack)
    } else {
        // Can't determine type - set to NIL (conservative)
        stack_module.setTopOfStack(vm, type_check_module.NIL_PTR);
    }
}
