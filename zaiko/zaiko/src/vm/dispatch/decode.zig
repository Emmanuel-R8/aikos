...
pub fn decodeInstructionFromMemory(vm: *VM, pc: LispPTR) errors.VMError!?Instruction {
    if (vm.virtual_memory) |vmem| {
        // Fetch opcode byte directly from virtual_memory slice
        if (pc >= vmem.len) {
            return null; // Invalid address - beyond virtual memory
        }

        const opcode_byte = vmem[@as(usize, @intCast(pc))];
        ...
        if (length > 1) {
            // Read operand bytes directly from virtual_memory slice
            for (0..length - 1) |i| {
                const operand_offset = @as(usize, @intCast(pc + @as(LispPTR, @intCast(i + 1))));
                operands_buffer[i] = vmem[operand_offset];
            }
