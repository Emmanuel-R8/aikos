...
pub fn initializeVMState(
    ...
    // USER HYPOTHESIS: All addresses should be bytes everywhere (instructions are single bytes)
    // Testing: Treat FX_FNHEADER as byte offset (cast Lisp_world to char* first)
    // This matches user observation: PC = 0x307898 = FX_FNHEADER (0x307864) + 0x34
    // If NativeAligned4FromLAddr should be: (char *)Lisp_world + LAddr (byte addressing)
    // Then: FuncObj = 0x307864 bytes, PC = FuncObj + CURRENTFX->pc
    const funcobj_offset_calc: usize = @as(usize, @intCast(fnheader_addr)); // Treat as byte offset
    ...
    if (funcobj_offset_opt) |funcobj_byte_offset| {
        ...
        // USER HYPOTHESIS: All addresses are bytes
        // Testing: CURRENTFX->pc should be byte offset directly (no division by 2)
        // But user observation shows: PC = FX_FNHEADER + 0x34, where 0x34 = 104/2
        // This suggests CURRENTFX->pc might be stored incorrectly, or there's a conversion
        // For now, testing both: direct byte offset and divided by 2
        const frame_pc_bytes_direct = @as(usize, @intCast(frame_pc)); // Direct byte offset
        const frame_pc_bytes_divided = frame_pc_bytes_direct / 2; // Divided by 2 (DLword to byte)
        std.debug.print("  CURRENTFX->pc as direct byte offset = {} bytes\n", .{frame_pc_bytes_direct});
        std.debug.print("  CURRENTFX->pc / 2 = {} bytes (testing DLword->byte conversion)\n", .{frame_pc_bytes_divided});

        // Test both: direct and divided
        // User observation suggests divided by 2 works: PC = FuncObj + 52 = 0x307898
        const calculated_pc = funcobj_byte_offset + frame_pc_bytes_divided;

        std.debug.print("  PC = FuncObj + (CURRENTFX->pc / 2) = 0x{x} + {} = 0x{x}\n", .{ funcobj_byte_offset, frame_pc_bytes_divided, calculated_pc });
