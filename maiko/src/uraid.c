...
LispPTR uraid_commands(void)
{
  ...
  case 'F':
  { /* print array block free list head(s) */
    long size;
  ...
  case 'm': /* MOVD */
    ...
    break;
  case 'O':
  { /* print instance from Laddr. Not documented */
    LispPTR objaddr;
    if (URaid_argnum == 1)
    {
...
#define XDUMPW 8
    ...
    ***/
...

    {
      ...
      while (ptr < endptr)
      {
        ...
        for (i = 0; ((ptr < endptr) && (i < XDUMPW)); ptr++, i++)
        {
          printf("%4x ", *ptr);
        }
        putchar('\n');
  ...
  case 's': /* SHELL */
    ...
    {
    case -1:
      (void)(void)fprintf(stderr, "uraid: Fork failed.\n");
      exit(1);

    case 0:
      (void)execl("/bin/sh", "sh", "-i", NULL);
      exit(1);
