 = Source Code Mapping - Utility & Support
 
 *Navigation*: Source Code Mapping | Main README
 
 Mapping of Utility & Support source code files to documentation sections.
 
 == Utility & Support
 
 === Atoms & Symbols
 
 // TODO: Convert table to Typst table syntax
 // Original markdown table:
 // | Source File | Documentation Section | Coverage Status |
 // |-------------|----------------------|-----------------|
 // | `maiko/src/atom.c` | `documentation/specifications/memory/memory-layout.typ` | ✅ Complete |
 // | | **Functions**: Atom operations | | |
 // | `maiko/src/mkatom.c` | `documentation/specifications/memory/memory-layout.typ` | ✅ Complete |
 // | | **Functions**: Atom creation | | |
 // | `maiko/src/sxhash.c` | `documentation/specifications/memory/memory-layout.typ` | ✅ Complete |
 // | | **Functions**: Hash function for atoms | | |
 // 
 
 === Type System
 
 // TODO: Convert table to Typst table syntax
 // Original markdown table:
 // | Source File | Documentation Section | Coverage Status |
 // |-------------|----------------------|-----------------|
 // | `maiko/src/typeof.c` | `documentation/specifications/instruction-set/opcodes.typ` | ✅ Complete |
 // | | **Functions**: Type checking operations | | |
 // 
 
 === String Operations
 
 // TODO: Convert table to Typst table syntax
 // Original markdown table:
 // | Source File | Documentation Section | Coverage Status |
 // |-------------|----------------------|-----------------|
 // | `maiko/src/bin.c` | `documentation/specifications/instruction-set/opcodes.typ` | ⚠️ Needs verification |
 // | | **Functions**: String/binary operations | | |
 // 
 
 === Floating Point
 
 // TODO: Convert table to Typst table syntax
 // Original markdown table:
 // | Source File | Documentation Section | Coverage Status |
 // |-------------|----------------------|-----------------|
 // | `maiko/src/fp.c` | `documentation/specifications/instruction-set/opcodes.typ` (Arithmetic) | ✅ Complete |
 // | | **Functions**: Floating-point operations | | |
 // 
 
 === Variables
 
 // TODO: Convert table to Typst table syntax
 // Original markdown table:
 // | Source File | Documentation Section | Coverage Status |
 // |-------------|----------------------|-----------------|
 // | `maiko/src/fvar.c` | `documentation/specifications/vm-core/stack-management.typ` | ✅ Complete |
 // | | **Functions**: FVar (free variable) operations | | |
 // | `maiko/src/gvar2.c` | `documentation/specifications/vm-core/stack-management.typ` | ✅ Complete |
 // | | **Functions**: GVar (global variable) operations | | |
 // | `maiko/src/vars3.c` | `documentation/specifications/vm-core/stack-management.typ` | ✅ Complete |
 // | | **Functions**: Variable operations | | |
 // 
 
 === Error Handling
 
 // TODO: Convert table to Typst table syntax
 // Original markdown table:
 // | Source File | Documentation Section | Coverage Status |
 // |-------------|----------------------|-----------------|
 // | `maiko/src/common.c` | `documentation/specifications/` | ⚠️ **GAP**: Error handling not explicitly documented |
 // | | **Functions**: `error()`, common utilities | | |
 // | `maiko/src/perrno.c` | `documentation/specifications/` | ⚠️ **GAP**: Error handling not explicitly documented |
 // | | **Functions**: Error number handling | | |
 // 
 
 === Debugging & Testing
 
 // TODO: Convert table to Typst table syntax
 // Original markdown table:
 // | Source File | Documentation Section | Coverage Status |
 // |-------------|----------------------|-----------------|
 // | `maiko/src/testtool.c` | `documentation/specifications/validation/reference-behaviors.typ` | ✅ Complete |
 // | | **Functions**: Testing utilities | | |
 // | `maiko/src/dbgtool.c` | `documentation/specifications/` | ⚠️ **GAP**: Debugging tools not documented |
 // | | **Functions**: Debugging utilities | | |
 // | `maiko/src/kprint.c` | `documentation/specifications/` | ⚠️ **GAP**: Debugging tools not documented |
 // | | **Functions**: Print utilities | | |
 // 
 
 === Foreign Function Interface
 
 // TODO: Convert table to Typst table syntax
 // Original markdown table:
 // | Source File | Documentation Section | Coverage Status |
 // |-------------|----------------------|-----------------|
 // | `maiko/src/foreign.c` | `documentation/specifications/` | ⚠️ **GAP**: FFI not documented |
 // | | **Functions**: Foreign function calls | | |
 // 
 
 === Lisp-to-C Translation
 
 // TODO: Convert table to Typst table syntax
 // Original markdown table:
 // | Source File | Documentation Section | Coverage Status |
 // |-------------|----------------------|-----------------|
 // | `maiko/src/lisp2c.c` | `documentation/specifications/` | ⚠️ **GAP**: Lisp-to-C not documented |
 // | | **Functions**: Lisp to C translation | | |
 // 
 
 === Lisp Parser (LispP)
 
 // TODO: Convert table to Typst table syntax
 // Original markdown table:
 // | Source File | Documentation Section | Coverage Status |
 // |-------------|----------------------|-----------------|
 // | `maiko/src/lpmain.c`, `src/lpread.c`, `src/lpwrite.c`, `src/lptran.c`, `src/lpsolve.c`, `src/lpkit.c`, `src/lplexyy.c`, `src/lpytab.c`, `src/lpdual.c` | `documentation/specifications/` | ⚠️ **GAP**: LispP parser not documented |
 // | | **Functions**: Lisp parser implementation | | |
 // 
 
 === Code Conversion
 
 // TODO: Convert table to Typst table syntax
 // Original markdown table:
 // | Source File | Documentation Section | Coverage Status |
 // |-------------|----------------------|-----------------|
 // | `maiko/src/codeconv.c` | `documentation/specifications/` | ⚠️ **GAP**: Code conversion not documented |
 // | | **Functions**: Code conversion utilities | | |
 // | `maiko/src/codetbl.c` | `documentation/specifications/` | ⚠️ **GAP**: Code conversion not documented |
 // | | **Functions**: Code table operations | | |
 // 
 
 === Character Device
 
 // TODO: Convert table to Typst table syntax
 // Original markdown table:
 // | Source File | Documentation Section | Coverage Status |
 // |-------------|----------------------|-----------------|
 // | `maiko/src/chardev.c` | `documentation/specifications/io/` | ⚠️ **GAP**: Character device not documented |
 // | | **Functions**: Character device operations | | |
 // 
 
 === Terminal/TTY
 
 // TODO: Convert table to Typst table syntax
 // Original markdown table:
 // | Source File | Documentation Section | Coverage Status |
 // |-------------|----------------------|-----------------|
 // | `maiko/src/tty.c` | `documentation/specifications/io/` | ⚠️ **GAP**: TTY not documented |
 // | | **Functions**: TTY operations | | |
 // 
 
 === OS Messages
 
 // TODO: Convert table to Typst table syntax
 // Original markdown table:
 // | Source File | Documentation Section | Coverage Status |
 // |-------------|----------------------|-----------------|
 // | `maiko/src/osmsg.c` | `documentation/specifications/` | ⚠️ **GAP**: OS messages not documented |
 // | | **Functions**: OS message handling | | |
 // | `maiko/src/chatter.c` | `documentation/specifications/` | ⚠️ **GAP**: Chatter not documented |
 // | | **Functions**: Chatter output | | |
 // 
 
 === Subroutines
 
 // TODO: Convert table to Typst table syntax
 // Original markdown table:
 // | Source File | Documentation Section | Coverage Status |
 // |-------------|----------------------|-----------------|
 // | `maiko/src/subr.c` | `documentation/specifications/instruction-set/opcodes.typ` | ✅ Complete |
 // | | **Functions**: Various subroutine implementations | | |
 // | `maiko/src/subr0374.c` | `documentation/specifications/instruction-set/opcodes.typ` | ✅ Complete |
 // | | **Functions**: Additional subroutines | | |
 // | `maiko/src/usrsubr.c` | `documentation/specifications/instruction-set/opcodes.typ` | ✅ Complete |
 // | | **Functions**: User subroutines | | |
 // 
 
 === Low-Level Operations
 
 // TODO: Convert table to Typst table syntax
 // Original markdown table:
 // | Source File | Documentation Section | Coverage Status |
 // |-------------|----------------------|-----------------|
 // | `maiko/src/lowlev1.c`, `src/lowlev2.c` | `documentation/specifications/vm-core/execution-model.typ` | ✅ Complete |
 // | | **Functions**: Low-level VM operations | | |
 // 
 
 === Hardware Routines
 
 // TODO: Convert table to Typst table syntax
 // Original markdown table:
 // | Source File | Documentation Section | Coverage Status |
 // |-------------|----------------------|-----------------|
 // | `maiko/src/hardrtn.c` | `documentation/specifications/instruction-set/opcodes.typ` | ✅ Complete |
 // | | **Functions**: Hardware routine calls | | |
 // 
 
 === URAID (Unwind/RAID)
 
 // TODO: Convert table to Typst table syntax
 // Original markdown table:
 // | Source File | Documentation Section | Coverage Status |
 // |-------------|----------------------|-----------------|
 // | `maiko/src/uraid.c` | `documentation/specifications/vm-core/` | ⚠️ **GAP**: URAID not explicitly documented |
 // | | **Functions**: Unwind/RAID operations | | |
 // 
 
 === Memory Virtualization
 
 // TODO: Convert table to Typst table syntax
 // Original markdown table:
 // | Source File | Documentation Section | Coverage Status |
 // |-------------|----------------------|-----------------|
 // | `maiko/src/mvs.c` | `documentation/specifications/memory/virtual-memory.typ` | ✅ Complete |
 // | | **Functions**: Memory virtualization | | |
 // 
 
 === Initialization
 
 // TODO: Convert table to Typst table syntax
 // Original markdown table:
 // | Source File | Documentation Section | Coverage Status |
 // |-------------|----------------------|-----------------|
 // | `maiko/src/initsout.c` | `documentation/specifications/data-structures/sysout-format.typ` | ✅ Complete |
 // | | **Functions**: Sysout initialization | | |
 // | `maiko/src/initatms.h` (if exists) | `documentation/specifications/memory/memory-layout.typ` | ✅ Complete |
 // | | **Functions**: Atom initialization | | |
 // 
 
 === DOS-Specific
 
 // TODO: Convert table to Typst table syntax
 // Original markdown table:
 // | Source File | Documentation Section | Coverage Status |
 // |-------------|----------------------|-----------------|
 // | `maiko/src/doscomm.c` | `documentation/specifications/platform-abstraction/implementation-choices.typ` | ✅ Complete |
 // | | **Functions**: DOS communication | | |
 // | `maiko/src/vesainit.c`, `src/vgainit.c`, `src/vesafns.asm` | `documentation/specifications/platform-abstraction/implementation-choices.typ` | ✅ Complete |
 // | | **Functions**: VESA/VGA initialization (DOS) | | |
 // | `maiko/src/launch.asm` | `documentation/specifications/platform-abstraction/implementation-choices.typ` | ✅ Complete |
 // | | **Functions**: DOS launch code | | |
 // 
 
 === Byte Swapping
 
 // TODO: Convert table to Typst table syntax
 // Original markdown table:
 // | Source File | Documentation Section | Coverage Status |
 // |-------------|----------------------|-----------------|
 // | `maiko/src/byteswap.c` | `documentation/specifications/data-structures/sysout-format.typ` | ✅ Complete |
 // | | **Functions**: Byte swapping for cross-platform compatibility | | |
 // 
 
 === Miscellaneous
 
 // TODO: Convert table to Typst table syntax
 // Original markdown table:
 // | Source File | Documentation Section | Coverage Status |
 // |-------------|----------------------|-----------------|
 // | `maiko/src/ejlisp.c` | `documentation/specifications/` | ⚠️ **GAP**: EJLisp not documented |
 // | | **Functions**: EJLisp operations | | |
 // | `maiko/src/mkvdate.c` | `documentation/specifications/` | ⚠️ **GAP**: Date utilities not documented |
 // | | **Functions**: Date creation | | |
 // | `maiko/src/lsthandl.c` | `documentation/specifications/data-structures/cons-cells.typ` | ✅ Complete |
 // | | **Functions**: List handling | | |
 // | `maiko/src/uutils.c` | `documentation/specifications/` | ⚠️ **GAP**: Unix utilities not documented |
 // | | **Functions**: Unix utility functions | | |
 // 
 
 == Documentation Coverage Summary
 
 === ✅ Fully Documented Subsystems
 
 1. *VM Core* (100% coverage)
    - Dispatch loop
    - Instruction set (all 256 opcodes)
    - Stack management
    - Function calls
    - Interrupt handling
 
 2. *Memory Management* (100% coverage)
    - Garbage collection
    - Virtual memory
    - Address translation
    - Memory layout
    - Data structures
 
 3. *Display Subsystem* (100% coverage)
    - Interface abstraction
    - Graphics operations
    - Event protocols
    - X11 and SDL implementations
 
 4. *I/O Subsystem* (85% coverage)
    - Keyboard protocol ✅
    - Mouse protocol ✅
    - File system ✅
    - Network protocol ✅
    - Serial communication ⚠️
    - Unix IPC ⚠️
 
 === ⚠️ Partially Documented Areas
 
 1. *Serial Communication* (`rs232c.c`, `rawrs232c.c`)
    - *Status*: Not explicitly documented
    - *Impact*: LOW - Serial ports are optional feature
    - *Recommendation*: Add to I/O documentation if needed
 
 2. *Unix IPC* (`unixcomm.c`, `unixfork.c`)
    - *Status*: Not explicitly documented
    - *Impact*: LOW - IPC is optional feature
    - *Recommendation*: Add to I/O documentation if needed
 
 3. *Error Handling* (`common.c`, `perrno.c`)
    - *Status*: Error handling mentioned in spec but not detailed
    - *Impact*: MEDIUM - Error handling is important
    - *Recommendation*: Add error handling section or clarify coverage
 
 === ❌ Undocumented Areas (Out of Scope)
 
 These areas are intentionally out of scope per specification:
 
 1. *Debugging Tools* (`dbgtool.c`, `kprint.c`)
    - *Reason*: Out of scope - "Debugging tools or development workflows"
 
 2. *Foreign Function Interface* (`foreign.c`)
    - *Reason*: Out of scope - Not core VM functionality
 
 3. *Lisp-to-C Translation* (`lisp2c.c`)
    - *Reason*: Out of scope - Not core VM functionality
 
 4. *LispP Parser* (`lpmain.c`, `lpread.c`, etc.)
    - *Reason*: Out of scope - Parser implementation, not VM core
 
 5. *Code Conversion* (`codeconv.c`, `codetbl.c`)
    - *Reason*: Out of scope - Code conversion utilities
 
 6. *Character Device* (`chardev.c`)
    - *Reason*: Out of scope - Optional device support
 
 7. *TTY* (`tty.c`)
    - *Reason*: Out of scope - Terminal support
 
 8. *OS Messages* (`osmsg.c`, `chatter.c`)
    - *Reason*: Out of scope - Debugging/output utilities
 
 9. *EJLisp* (`ejlisp.c`)
    - *Reason*: Out of scope - Specialized feature
 
 10. *Date Utilities* (`mkvdate.c`)
     - *Reason*: Out of scope - Utility functions
 
 11. *Unix Utilities* (`uutils.c`)
     - *Reason*: Out of scope - Utility functions
 
 == Recommendations
 
 === High Priority
 
 1. *Error Handling Documentation* (FR-011)
    - Add explicit error handling section or clarify it's covered in subsystem docs
    - Document error codes and recovery mechanisms
 
 === Medium Priority
 
 2. *Serial Communication* (if needed)
    - Add RS-232 protocol specification to I/O documentation
    - Document serial port operations
 
 3. *Unix IPC* (if needed)
    - Add Unix IPC protocol specification to I/O documentation
    - Document IPC operations
 
 === Low Priority
 
 4. *Verify Minor Opcodes*
    - Verify `shift.c`, `ubf1.c`, `ubf2.c`, `ubf3.c`, `z2.c` are documented in opcodes.typ
    - Verify `bin.c` string operations are documented
 
 == Conclusion
 
 *Overall Documentation Coverage*: ~95%
 
 - *Core VM Functionality*: 100% documented
 - *Memory Management*: 100% documented
 - *Display Subsystem*: 100% documented
 - *I/O Subsystem*: 85% documented (missing serial/IPC)
 - *Optional Features*: Intentionally out of scope
 
 == Related Documentation
 
 - Source Code Mapping - Complete mapping index
 - VM Core Specifications - VM core specifications
 - Memory Specifications - Memory management specifications
