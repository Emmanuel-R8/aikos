...
- Supports standard Unix path conventions - Used by `medley.sh` wrapper script pointerSource Code: medley/scripts/medley/medley_run.sh
...
2. *medley_args.sh* - Argument parsing logic
  - Config file processing
  - Argument validation

3. *medley_run.sh* - Runtime execution logic
  - Maiko invocation
  - Environment setup

4. *medley_configfile.sh* - Config file parsing
...
6. *medley_usage.sh* - Usage message generation
  - Help text
...
==== Display Flags - `-g WxH, --geometry WxH`: Window geometry
...
==== Other Flags - `-h, --help`: Show help message
- `-z, --man`: Show man page
...
3. `MEDLEYDIR/.medley_config`: Medley directory config pointerSource Code Reference: medley/scripts/medley/medley_configfile.sh - config file resolution
...
*Source Code Reference*: medley/scripts/medley/medley_args.sh - LDEREMCM setup pointerSee: Interface - Environment Variables for complete environment variable documentation
...
- Cleanup: Handle cleanup on errors pointerSource Code Reference: medley/scripts/medley/medley_run.sh - error handling
...
=== WSL - Uses `medley_run.sh` with VNC support
