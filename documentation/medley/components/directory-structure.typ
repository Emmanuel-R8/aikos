...
- `scripts/loadups/`: Loadup scripts for creating sysout files
  ...
  - `loadup-db.sh`: Stage 7 only - `loadup-full.sh`: Stages 1-4 only pointerSee: Scripts Component for script system documentation
...
- `SIMPLE-INIT`: Simple greet file - `NOGREET`: Disable greet file execution pointerPurpose: Initialize Lisp environment before main system starts.
...
- `docs/ReleaseNote/`: Release notes - `docs/Sun Users Guide/`: Sun Users Guide pointerPurpose: User and developer documentation.
...
- `unicode/vendors/`: Vendor-specific mappings - `unicode/xerox/`: Xerox character mappings pointerPurpose: Unicode support data.
...
- `installers/downloads_page/`: Downloads page generation pointerPurpose: Platform-specific installation tools.
...
==== Configuration Files - `.medley_config`: User-specific config file pointerPurpose: User-specific default command-line arguments.

*See*: Configuration Files Component for config file documentation

==== Greet Files - `INIT.LISP`: User-specific greet file pointerPurpose: User-specific initialization.

*See*: Greet Files Component for greet file documentation

== File Naming Conventions

=== Interlisp Source Files - *Naming*: UPPERCASE names, no file extensions
- *Examples*: `INIT`, `MAIN`, `SYSOUT`

=== Compiled Files - *Interlisp*: `.LCOM` extension
