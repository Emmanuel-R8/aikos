...
=== Variable Access
- *IVAR0-IVAR6* (0x40-0x46) [1] Push local variable 0-6.  - Uses LispPTR offset (index × 4 bytes)
...
- *PVAR0-PVAR6* (0x48-0x4E) [2] Push parameter 0-6.  - Uses LispPTR offset (index × 4 bytes)
...
- *PVARX_underscore* (0x5F) [2] Alternative indexed PVAR.
- *Operand*: `x` (1B, DLword offset)
- *CRITICAL*: Uses DLword offset, NOT LispPTR offset
...
- *IVARX_underscore* (0x62) [2] Set indexed local variable.
- *Operand*: `x` (1B, DLword offset)
- *CRITICAL*: Uses DLword offset, NOT LispPTR offset
- *C*: `IVARX_(x): * ((LispPTR)((DLword)IVAR + (x))) = TOPOFSTACK;`
- *Access*: Writes LispPTR to `(DLword)IVAR + x` (x is in DLword units)
- *Byte Order*: Writes in big-endian byte order for sysout format
- *GVAR_underscore* (0x63) [3] Atom index (2B). Set global variable value.
