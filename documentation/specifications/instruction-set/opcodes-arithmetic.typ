...
- C: `N_OP_iplusn` in `maiko/src/arithops.c` - *IDIFFERENCE_N* (0xDE) [2] Subtract constant N from TOS.
- Operand: n (1B, constant to subtract)
  - Subtracts constant n from TOS value
- Overflow checking matches C implementation
- C: `N_OP_idifferencen` in `maiko/src/arithops.c` - *BOXIPLUS* (0xDF) [1] Add number to FIXP box in place.
...
Base operations access memory at a base address plus an offset. All base addresses are masked with `POINTERMASK` (0xfffffff for BIGVM, 0xffffff for non-BIGVM) to extract the portion. - *GETBASEBYTE* (0xC2) [1] Read byte from memory.
...
- If validation fails, triggers UFN lookup - *GETBASE_N* (0xC8) [2] Read DLword from memory.
- Index: 1-byte operand
- Reads DLword (2 bytes, big-endian) from `(POINTERMASK & base) + index`
- Returns `S_POSITIVE | word_value` - *GETBASEPTR_N* (0xC9) [2] Read LispPTR from memory.
- Index: 1-byte operand
- Reads LispPTR (4 bytes, big-endian) from `(POINTERMASK & base) + index`
- Returns `POINTERMASK & pointer_value` - *PUTBASE_N* (0xCD) [2] Write DLword to memory.
- Index: 1-byte operand
...
- If validation fails, triggers UFN lookup - *PUTBASEPTR_N* (0xCE) [2] Write LispPTR to memory.
- Index: 1-byte operand
- Writes `POINTERMASK & pointer` as LispPTR (4 bytes, big-endian) to `(POINTERMASK & base) + index`
- Pushes `base` back on stack - *GETBITS_N_FD* (0xCA) [3] Extract bit field from memory.
...
- Returns `S_POSITIVE | bit_field` - *PUTBITS_N_FD* (0xCB) [3] Write bit field to memory.
- Operands: offset (1B), field descriptor (1B)
  - `value` must be `S_POSITIVE` (small integer)
...
== Address Manipulation - *ADDBASE* (0xD0) [1] Add two base addresses.
- Applies `POINTERMASK` to both operands before addition
- Returns `POINTERMASK &* (a_ptr + b_ptr)` - *HILOC* (0xD2) [1] Extract high 16 bits.
- Returns `S_POSITIVE | GetHiWord(value)` where `GetHiWord(x) = (x >> 16) & 0xFFFF` - *LOLOC* (0xD3) [1] Extract low 16 bits.
- Returns `S_POSITIVE | GetLoWord(value)` where `GetLoWord(x) = x & 0xFFFF` - *BASE_LESSTHAN* (0xCF) [1] Compare base addresses.
- Applies `POINTERMASK` to both operands before comparison
- Returns `S_POSITIVE | 1` (T) if `(POINTERMASK & a) < (POINTERMASK & b)`, else `0` (NIL)

== GC Operations - *GCREF* (0x15) [2] Ref type (1B). TOS = object. ADDREF/DELREF/STKREF based on type.
