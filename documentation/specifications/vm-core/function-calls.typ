...
*Instruction Format*: - *Length*: 3 bytes for non-BIGATOMS (FN_OPCODE_SIZE = 3)
...
*Call Algorithm*: [`function ExecuteFN(arg_count, atom_index`]:
...
*Call Algorithm*: [`function ExecuteFNX(`]:
...
[`function GetFunctionObject(atom_index`]:
    ...
    // - def: 28 bits (BIGVM) or 24 bits (non-BIGVM) - to function header
...
[`function SetupArguments(function_obj, arg_count`]:
    ...
    while pvar_count > 0:
        CurrentStackPTR[0] = 0x0ffff0000  // Initialize PVar
...
[`function TransferControl(function_obj`]:
    ...
    // Set IVar IVar = NativeAligned2FromStackOffset(CurrentFrame.nextblock)

    // Set PVar PVar = CurrentStackPTR + FRAMESIZE
...
*Return Algorithm*: [`function ExecuteRETURN(`]:
    // Get return value
...
[`function RestoreExecutionState(frame`]:
    ...
    // Restore stack CurrentStackPTR = CalculateStackPointer(frame)
...
=== Argument Types pointerFixed Arguments: - Arguments passed on stack
- Count matches function definition
- Accessed via PVar pointerSpread Arguments: - Variable number of arguments
