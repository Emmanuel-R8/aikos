...
*Date*: 2026-02-19 16:03
*Purpose*: Document the removal of obsolete platform-specific files from the Maiko C implementation

== Overview

This document records the removal of obsolete platform-specific source files that were no longer compiled or maintained as part of the Maiko C emulator. These files were remnants of historical platform support that has been superseded by modern alternatives.

== Removed Files

=== DOS Platform Files (3 files)

The following DOS-specific platform files were removed. DOS is no longer a supported platform for Medley Interlisp.
...
The following assembly language files were removed. These were platform-specific optimizations for architectures that are no longer primary targets.
...
#table(
  ...
  [`maiko/src/dspSPARC.il`], [SPARC], [Inline assembly for display], [SPARC architecture deprecated],
)
...
#table(
  columns: (auto, auto, 1fr),
  [*File*], [*Purpose*], [*Reason for Removal*],
  [`maiko/src/vesainit.c`], [VESA VBE display initialization], [VESA display deprecated; SDL is current backend],
  [`maiko/src/vgainit.c`], [VGA display initialization], [VGA display deprecated; SDL is current backend],
)
...
All removed files were already excluded from the CMake build configuration (`maiko/CMakeLists.txt`). The build was verified after removal:
...
- #link("../../specifications/platform-abstraction/implementation-choices.typ")[Platform Abstraction: Implementation Choices]
