...
=== Dispatch Loop - `src/xc.c`: Main bytecode dispatch loop
...
=== Stack Management - `src/llstk.c`: Low-level stack operations
...
==== Arithmetic Operations - `src/arithops.c`: Arithmetic opcodes (add, subtract, multiply, divide) - `src/fp.c`: Floating-point operations
...
==== Control Flow - `src/loopsops.c`: Loop operations
...
[`    DLword alink;        // Activation link (previous frame`]
    DLword fnheader;     // Function header DLword pc;           // Program counter offset
...
[`struct state {`]
[`    DLword pointerivar;           // IVar pointer`]
[`    DLword pointerpvar;           // PVar pointer`]
[`    DLword pointercsp;            // C stack pointer`]
[`    ByteCode pointercurrentpc;    // Current program counter`]
[`    struct fnhead pointercurrentfunc;  // Current function`]
[`    DLword pointerendofstack;     // End of stack`]
...
=== Special Operations (0xC0-0xFF) - Type checking
- Cell creation
