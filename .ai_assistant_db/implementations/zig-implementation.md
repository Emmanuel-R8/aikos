...
- 🔄 **VM Execution** (P1 - In Progress)
  ...
  - **CRITICAL BLOCKER**: The initial frame in `starter.sysout` at `currentfxp=0x2e72` (11890 DLwords from Stackspace, byte offset 0x25ce4) is **completely uninitialized** (all zeros): `fnheader=0`, `nextblock=0`, `pc=0`. **DEBUG FINDINGS**: (1) Direct hexdump inspection confirms frame contains all zeros at calculated offset. (2) Test program `test_fastretcall.c` confirms: `nextblock=0` would cause `start_lisp()` to error when checking `GETWORD(next68k) != STK_FSB_WORD` (Stackspace base contains `0x00f1`, not `0xA000`). (3) Test program confirms: `fnheader=0` would cause `FastRetCALL` to point `FuncObj` to invalid memory (Lisp_world base). (4) **C emulator runs successfully**, proving it MUST initialize frame somewhere. (5) Checked `init_storage()`, `init_ifpage()`, `init_iopage()` - none initialize frame. **HYPOTHESIS**: Frame must be initialized during sysout page loading (when pages are mapped), OR `start_lisp()` has hidden initialization logic, OR frame location calculation is incorrect. **NEXT STEP**: Rebuild C emulator with debug statements to see actual values at runtime, or trace through page loading to find frame initialization.
