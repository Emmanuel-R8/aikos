...
**Primary Script**: `medley_run.sh` (with VNC support)
...
**Source Code Reference**: [medley/scripts/medley/medley_run.sh](medley/scripts/medley/medley_run.sh)
...
**Source Code Reference**: [medley/scripts/medley/medley_main.sh](medley/scripts/medley/medley_main.sh) - WSL detection
...
sequenceDiagram
    ...
    Script->>Xvnc: Start Xvnc server
    Xvnc-->>Script: Display number
    Script->>VNCViewer: Start VNC viewer (Windows)
    Script->>Maiko: Invoke with VNC display
    Maiko->>Xvnc: Connect to display
    Xvnc-->>VNCViewer: Forward display
```
...
5. **Invoke Maiko**: Script invokes Maiko with VNC display

**Source Code Reference**: [medley/scripts/medley/medley_vnc.sh](medley/scripts/medley/medley_vnc.sh) - VNC implementation
...
**Purpose**: Useful when calling Medley as part of automation scripts where Medley may run for very short time (< a couple of seconds).

**Source Code Reference**: [medley/docs/man-page/medley.1.md](medley/docs/man-page/medley.1.md) - automation flag
...
- **Location**: `%USERPROFILE%\AppData\Local\Interlisp\vncviewer.exe`
- **Installation**: Scripts may copy VNC viewer to Windows directory if needed

**Source Code Reference**: [medley/scripts/medley/medley_vnc.sh](medley/scripts/medley/medley_vnc.sh) - VNC viewer location
...
- **WSL1**: VNC flag always set (cannot be disabled)
- **WSL2**: VNC flag can be enabled/disabled with `-v, --vnc` flag
...
**Platform Identifier**: `linux.x86_64` (same as Linux)
